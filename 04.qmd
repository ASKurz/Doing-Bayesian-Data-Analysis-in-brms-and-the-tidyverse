...
### Discrete distributions: Probability mass {#sec-Discrete-distributions-Probability-mass}
...
In order to recreate Figure 4.2, we need to generate the heights data with `height_weight_generator()`, our version of Kruschke's `HtWtDataGenerator()` function, which we introduced back in @sec-The-steps-of-Bayesian-data-analysis.
...
#### Properties of probability density functions {#sec-Properties-of-probability-density-functions}
...
### Highest density interval (HDI) {#sec-Highest-density-interval-HDI}
...
As far as I could tell, Figure 4.5.b is of a beta distribution, which Kruschke covered in greater detail starting in @sec-Inferring-a-Binomial-Probability-via-Exact-Mathematical-Analysis. I got the `shape1` and `shape2` values from playing around. Here's the new data grid.
...
## Two-way distributions {#sec-Two-way-distributions}
