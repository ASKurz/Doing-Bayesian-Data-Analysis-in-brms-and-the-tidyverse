# The R Programming Language {#sec-The-R-Programming-Language}
...
I don't tend to save lists of commands in text files. Rather, I almost exclusively work within [R Notebook](https://bookdown.org/yihui/rmarkdown/notebook.html) files, which I discuss more fully in @sec-Programming-in-R. As far as *sourcing*, I never use the `source()` approach Kruschke discussed in page 36. I'm not opposed to it. It's just not my style.
...
we made our tibble. In **R**, data frames are one of the primary types of data objects (see @sec-List-and-data-frame, below). We'll make extensive use of data frames in this project. Tibbles are a particular type of data frame, which you might learn more about in the [tibbles section](https://r4ds.had.co.nz/tibbles.html) of Grolemund and Wickham's [-@grolemundDataScience2017] *R4DS*. With those first two lines, we determined what the name of our tibble would be, `d`, and made the first column, `x`.

Note the `|>` operator at the end of the second line. In prose, we call that the *pipe*. As explained in [Section 4.3 of *R4DS2*](https://r4ds.hadley.nz/workflow-style.html#sec-pipes), "a good way to pronounce `|>` when reading code is 'then.'" So in words, the those first two lines indicate "Make an object, `d`, which is a tibble with a variable, `x`, defined by the `seq()` function, *then*..."
...
In contrast to Kruschke's preference, I will use the [arrow operator, `<-`, to assign](https://style.tidyverse.org/syntax.html#assignment) values to named variables.^[The whole `=` versus `<-` controversy can spark some strong opinions within the **R** community. If you're curious about the historical roots for `<-`, check out [Colon Fay](https://twitter.com/_ColinFay)'s nice blog post, [*Why do we use arrow as an assignment operator?*](https://colinfay.me/r-assignment/).]
...
If you have a sole number, a *scalar*, you can multiply an entire vector by it like:
...
Factors can come in very handy when modeling with certain kinds of categorical variables, as in @sec-Nominal-Predicted-Variable, or when arranging elements within a plot.
...
### List and data frame {#sec-List-and-data-frame}
...
## Programming in R {#sec-Programming-in-R}
...
### Running a program {#sec-Running-a-program}
...
Here's Kruschke's initial `if()` and `else()` example.
...
Change the value you've assigned to `x` to see what might happen.
...
### Measuring processing time {#sec-Measuring-processing-time}
