...
We’ll use the `filter()` function to take the two slices from the posterior grid. Remember that `sum_of_prior` value we saved a little while back? It finally comes into play, here. If we multiply our `prior` variable by that, we can renormalize the two slices "so that they are individually proper probability densities that sum to 1.0 over $\theta$" (p. 226).
...
From formula 9.1, the posterior $p(\theta, \omega| D)$ is proportional to $p(D | \theta) p(\theta|\omega) p(\omega)$. Divide by the normalizing constant and we'll have it in a proper probability metric. Recall that we've already saved the results of $p(\theta|\omega) p(\omega)$ in the `prior` column. So we just need to multiply `prior` by `likelihood` and divide by their sum.
...
Note that after we slice with `filter()`, the next two wrangling lines renormalize those posterior slices into probability metrics. That is, when we take a slice through the joint posterior at a particular value of $\omega$, and renormalize by dividing the sum of discrete probability masses in that slice, we get the conditional distribution $p(\theta | \omega, D)$ (p. 228).
