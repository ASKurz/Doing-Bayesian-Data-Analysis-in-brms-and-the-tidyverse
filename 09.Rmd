...
$$p(D | \theta_1, \theta_2) = \bigg ( \theta_1^{z_1} (1 - \theta_1) ^ {N_1 - z_1} \bigg ) \bigg ( \theta_2^{z_2} (1 - \theta_2) ^ {N_2 - z_2} \bigg )$$
...
$$\text{logit} (\theta) = \text{log} (\theta/(1 - \theta))$$
...
We'll need `brms::inv_logit_scaled()` to convert the model parameters to predict $\theta$ rather than $\text{logit} (\theta)$. After the conversions, we'll be ready to make the histograms in the lower portion of Figure 9.10.
...
```{r fit1_prior, cache = T, warning = F, message = F, results = 'hide'}
fit1_prior <-
  brm(data = my_data,
      family = bernoulli(link = logit),
      y ~ 1 + (1 | s),
