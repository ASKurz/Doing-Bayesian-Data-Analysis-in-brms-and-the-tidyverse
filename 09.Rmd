...
The trace plots indicate no problems with convergence. We'll need to extract the posterior draws with `as_draws_array()` and open the **bayesplot** package before we can examine the autocorrelations.

```{r, warning = F, message = F}
draws_array <- as_draws_array(fit9.1)
...
draws_array %>% 
  mcmc_acf(pars = c("b_Intercept", "sd_s__Intercept"), lags = 10) +
  theme_cowplot()
```
...
We'll need `brms::inv_logit_scaled()` to convert the model parameters to predict $\theta$ rather than $\operatorname{logit}(\theta)$. After the conversions, we'll be ready to make the histograms in the lower portion of Figure 9.10. Before all that, we'll first extract the posterior draws with the `as_draws_df()` function to get them in a nice data frame format appropriate for **tidyverse**-style wrangling

```{r, fig.width = 8, fig.height = 4, warning = F, message = F}
# extract the posterior draws
draws_df <- as_draws_df(fit9.1)
...
And here we'll take a subset of the columns in `prior_draws_df`, transform the results to the probability metric, and save.
...
Now we can use our `prior_draws_df` object to make the diagonal of the lower grid of Figure 9.11.
...
```{r}
cor(prior_draws_df) %>% round(digits = 2)
```
...
```{r, fig.width = 8, fig.height = 4, warning = F, message = F}
draws_array <- as_draws_array(fit9.2)

draws_array %>% 
  mcmc_acf(pars = c("b_Intercept", 
                    "sd_PriPos__Intercept", 
                    "sd_PriPos:Player__Intercept"), 
           lags = 8) +
  theme_minimal_hgrid()
```
...
p1 <-
  make_histogram(data = draws_df_small,
                 aes(x = Pitcher), 
                 title = "Pitcher", 
                 xlim = c(.1, .25))
...
p3 <-
  make_point(data = draws_df_small,
