...
If you want to display each chain as its own density, you can use the handy `mcmc_dens_overlay()` function from the [**bayesplot** package](https://CRAN.R-project.org/package=bayesplot) to return the overlaid densities.
...
To complicate matters further, the bayesplot package also has an `rhat()` function, which is not currently applicable for objects of class `brmsfit`.
...
## Example: Difference of biases {#sec-Example-Difference-of-biases}
...
### Defining new likelihood functions {#sec-Defining-new-likelihood-functions}
