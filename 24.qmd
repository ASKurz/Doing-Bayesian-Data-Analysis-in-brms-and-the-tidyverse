# Count Predicted Variable {#sec-Count-Predicted-Variable}
...
```{r}
my_data |> 
  pivot_wider(names_from = Hair, values_from = Count)
```
...
Before we go any further, let's talk color and theme. For this chapter, we'll take our color palette from the [**ochRe** package](https://github.com/ropenscilabs/ochRe) [@R-ochRe], which provides several color palettes inspired by the art, landscapes, and wildlife of Australia. For the plots to follow, we will use the `"healthy_reef"` palette.^[At the moment (1-2-2025), loading the **ochRe** package is preventing `brm()` from working properly, with something to do with the **raster** package. This wasn't an issue for previous versions of the ebook. At the moment, the easy fix is to use the `ochRe::ochre_palettes` syntax rather than loading **ochRe**.]

```{r}
#| fig-height: 3

scales::show_col(ochRe::ochre_palettes[["healthy_reef"]])

hr <- ochRe::ochre_palettes[["healthy_reef"]]
```
...
y_log_mean; y_log_sd; s_r$shape; s_r$rate
```
...
np <- nuts_params(fit24.1)

plot(fit24.1)
pairs(fit24.1, off_diag_args = list(alpha = 1/4, size = 1/4), np = np)
```
...
bind_rows(
  ...
  labs(x = NULL,
       title = "Priors") +
  facet_wrap(~ prior, labeller = label_parsed, scales = "free")
```
