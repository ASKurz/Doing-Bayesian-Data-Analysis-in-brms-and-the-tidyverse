...
Before we make `p`, let's talk color and theme. For this chapter, we'll carry forward our practice from @sec-Metric-Predicted-Variable-with-One-Nominal-Predictor and take our color palette from the **palettetown** package. Our color palette will be #15, which is based on [Beedrill](https://www.pokemon.com/us/pokedex/beedrill).
...
p7 <- tibble(x = c(0.05, 0.34, 0.64, 0.945),
             y = 1,
...
# Half-normal density
p9 <- tibble(x = seq(from = 0, to = 3, by = 0.01)) |> 
  ggplot(aes(x = x, y = (dnorm(x)) / max(dnorm(x)))) +
  geom_area(fill = bd[6]) +
  annotate(geom = "text",
           x = 1.5, y = 0.2,
...
as_draws_df(fit20.1) |> 
  select(b_Intercept, 
         ...
         sigma) |> 
  summarise_draws(mean, median, mode = Mode, ess_bulk, ess_tail, ~ quantile(.x, probs = c(0.025, 0.975))) |> 
...
# Rope annotation
d_text <- tibble(name  = "Full - Assis @ PSY",
                 value = 10000,
                 y     = 0.95,
                 label = "ROPE") |> 
...
d <- tibble(monetary_units = c(10, 12, 15, 18),
...
# Define the data
d <- crossing(int = c("Non−crossover Interaction", "Crossover Interaction"),
              x1  = factor(1:2),
              x2  = factor(1:2)) |> 
...
# Half-normal density
p10 <- tibble(x = seq(from = 0, to = 3, by = 0.01)) |> 
  ggplot(aes(x = x, y = (dnorm(x)) / max(dnorm(x)))) +
  geom_area(fill = bd[6]) +
  annotate(geom = "text",
           x = 1.5, y = 0.2,
...
# Exponential density
p11 <- tibble(x = seq(from = 0, to = 1, by = 0.01)) |> 
...
# Normal density
p12 <- tibble(x = seq(from = -3, to = 3, by = 0.1)) |> 
  ggplot(aes(x = x, y = (dnorm(x)) / max(dnorm(x)))) +
  geom_area(fill = bd[6]) +
  annotate(geom = "text",
           x = 0, y = 0.2,
...
# Two annotated arrows
p16 <- tibble(x = c(0.18, 0.18),  # 0.2142857
              y = c(1, 0.45),
              xend = c(0.18, 0.67),
              yend = c(0.75, 0.14)) |>
  ggplot(aes(x = x, xend = xend,
...
# Combine, adjust, and display
(p1 + p1 + p1 + p3 + p4 + p5 + p6 + p2 + p2 + p2 + p8 + p7 + p9 + 
...
`r emo::ji("warning")` **Warning**: This one took the better part of two hours fit. `r emo::ji("warning")`

```{r}
#| echo: false
#| eval: false

# Heads up: this took 1.658 hours to fit (1-2-2026)
```
...
```{r}
fit20.1 <- add_criterion(fit20.1, criterion = "loo")
fit20.2 <- add_criterion(fit20.2, criterion = "loo")
```
...
```{r}
d <- tibble(response_time = c(300, 320, 350, 370, 400, 420, 450, 470),
...
fitted(fit20.3,
  ...
  transmute(
    ...
    `Moldbrd.Ridge\nvs\nChisel` = ((Moldbrd_Broad + Moldbrd_Deep + 
                                      Moldbrd_Surface + Ridge_Broad + Ridge_Deep + 
                                      Ridge_Surface) / 6) - ((Chisel_Broad + Chisel_Deep + Chisel_Surface) / 3),
    `Deep.Surface\nvs\nBroad` = ((Chisel_Deep + Moldbrd_Deep + 
                                    Ridge_Deep + Chisel_Surface + 
                                    Moldbrd_Surface + Ridge_Surface) / 6) - ((Chisel_Broad + Moldbrd_Broad + Ridge_Broad) / 3),
    `Chisel.Moldbrd.v.Ridge\n(x)\nBroad.v.Deep.Surface` = (((Chisel_Broad + Chisel_Deep + 
      ...
      (((Chisel_Broad + Moldbrd_Broad + Ridge_Broad) / 3) - 
         ((Chisel_Deep + Moldbrd_Deep + Ridge_Deep + Chisel_Surface + Moldbrd_Surface + Ridge_Surface) / 6))) |> 
  pivot_longer(cols = everything()) |> 
...
fit20.3 <- add_criterion(fit20.3, criterion = "loo")
fit20.4 <- add_criterion(fit20.4, criterion = "loo")
```

Executing that yielded the following warning message:

> Found 3 observations with a pareto_k > 0.7 in model 'fit20.3'. We recommend to set 'moment_match = TRUE' in order to perform moment matching for problematic observations. 
...
fit20.3mm <- brm(
  ...
  file = "fits/fit20.03mm")

fit20.3mm <- add_criterion(fit20.3mm, criterion = "loo")

fit20.3mm_mm <- loo_moment_match(fit20.3mm)
# Warning :Some Pareto k diagnostic values are too high. See help('pareto-k-diagnostic') for details.
``` 

To use the `moment_match` approach, the model in question must have been fit with the setting `save_pars = save_pars(all = TRUE)` within the `brm()` call. The default is `save_pars = NULL` and we used the default settings, above. If you go through the trouble of refitting the model with those updated settings, you'll find that the `moment_match` approach didn't help in this case. For the sake of reducing clutter, I'm not going to show the code for refitting the model. However, the next warning message I got after executing `fit20.3 <- add_criterion(fit20.3, criterion = "loo", moment_match = TRUE)` was:

> Warning :Some Pareto k diagnostic values are too high. See help('pareto-k-diagnostic') for details

It's not a bad idea to execute `help('pareto-k-diagnostic')` and browse though the options that pop up from the documentation, but I didn't find them particularly helpful in this case. So it's time to bring in the big guns. Let's use `reloo`. Be warned that, because this requires refitting the model multiple times, this will take a few minutes.^[To learn more about this method, read through the `reloo` section of the [brms reference manual](https://cran.r-project.org/web/packages/brms/brms.pdf).]

```{r}
#| echo: false
#| eval: false

# This took 8.768727 mins (1-2-2026)
fit20.3 <- add_criterion(fit20.3, criterion = "loo", reloo = TRUE, overwrite = TRUE)
t1 <- Sys.time()
t1 - t0
```
