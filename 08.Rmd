...
Note how the `brms::plot()` function simply took our model fit object as input. Other post-processing functions will require us to pass the posterior draws in the form of a data frame or an array. To get ready for them, here we'll save them in an array with the `as_draws_array()` function and as a data frame with the `as_draws_df()` function.

```{r}
draws_array <- as_draws_array(fit8.1)
draws_df <- as_draws_df(fit8.1) 
```
...
Now we're ready to use our `draws_array` object within the `mcmc_dens_overlay()` function to return the overlaid densities.
...
draws_array %>% 
  mcmc_acf(pars = "b_Intercept", lags = 35) +
  theme_minimal_hgrid()
```
...
When plotting the posterior distribution of a parameter estimated with **brms**, you typically do so working with the results of an object returned by one of the `as_draws_` functions. Recall we already saved those results as `draws_df` and `draws_array`. Here's a look at `draws_df`.

```{r}
head(draws_df)
```

With `draws_df` in hand, we can use **ggplot2** to do the typical distributional plots, such as with `geom_histogram()`.
...
my_breaks <-
  mode_hdi(draws_df$b_Intercept)[, 1:3] %>% 
...
```{r, fig.width = 10, fig.height = 3, message = F, warninf = F}
long_draws <-
  draws_df %>% 
...
```{r}
draws_df <- as_draws_df(fit8.3) %>% select(-lp__)
...
rm(p1, my_arrow, p2, p3, p4, p5, layout, my_data, fit8.1, draws_array, draws_df, fit8.1_c, estimate_mode, my_breaks, fit8.2, long_draws, fit8.3, d_line, levels)
```
