...
Note how we're continuing to use `theme_cowplot()`, which we introduced in the last chapter. Anyway, the remaining panels in Figure 7.1 require we simulate the data.
...
, fig.width = 8, fig.height = 2.75, warning = F, message = F
...
The modes are the points and the lines depict the 95% HDIs. Also, did you notice our use of the `cowplot::panel_border()` funciton? The settings from `theme_cowplot()` can make it difficult to differentiate among subplots when faceting. By throwing in a call to `panel_border()` after `theme_cowplot()`, we added in lightweight panel borders.
