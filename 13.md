...
To start our sequence of simulated coin flips, we’ll set the total
number of trials we’d like, `n_trial`, specify our `bias`, and set out
seed. Like Kruschke did in the text we’ll do `bias <- .5`, first.
...
Here’s a little custom function that will fit frequentist logistic
regression models for each combination of `n` and `z` and then extract
...
For the Bayes factors, Kruschke indicated these were computed based on
Equation 12.3 from page 344. Here’s the equation.
...
To ground ourselves, here’s some of the content from that page that
followed the equation.

> For a default alternative prior, the beta distribution is supposed to
> be uninformed, according to particular mathematical criteria.
> Intuition might suggest that a uniform distribution suits this
> requirement, that is, \(\text{beta} (\theta | 1, 1)\). Instead, some
> argue that the most appropriate uninformed beta distribution is
> \(\text{beta} (\theta | \epsilon, \epsilon)\), where \(\epsilon\) is a
> small number approaching zero (p. 344)

The \(\text{beta} (\epsilon, \epsilon)\), recall, is the Haldane prior.
Often times, \(\epsilon = 0.01\). That will be our approach here, too.
Let’s make another custom function.
...
To compute the HDIs for each iteration, we’ll want to use the
`hdi_of_qbeta()` function from chapters 10 and 12.
...
We’re finally ready to define the five subplots for our version of
Figure 13.4.
...
![](13_files/figure-gfm/unnamed-chunk-77-1.png)<!-- -->

Now let’s compute data of the same form, but based on \(\theta = .65\).
This time we’ll do all the data wrangling steps in one code block.
...
![](13_files/figure-gfm/unnamed-chunk-79-1.png)<!-- -->
...
```
    ...
    ## 
    ## Call:
    ## glm(formula = cbind(z, n - z) ~ 1, family = binomial(link = "logit"), 
    ##     data = tibble(z = z, n = n))
    ## 
    ## Deviance Residuals: 
    ## [1]  0
...
```
    ...
    ## 
    ## Call:
    ## glm(formula = y ~ 1, family = binomial(link = "logit"), data = tibble(y = rep(1:0, 
    ##     times = c(z, n - z))))
    ## 
    ## Deviance Residuals: 
...
```
    ...
    ##  [1] gridExtra_2.3   tidybayes_1.1.0 brms_2.9.0      Rcpp_1.0.1     
    ...
    ##  [63] dygraphs_1.1.1.6          pkgbuild_1.0.3           
    ...
    ##  [79] generics_0.0.2            pillar_1.4.1             
    ...
    ##  [93] threejs_0.3.1             digest_0.6.19            
