...
```
##   b_Intercept Intercept      lp__
## 1   0.7263424 0.7263424 -4.691665
## 2   0.7626307 0.7626307 -4.815941
## 3   0.7222605 0.7222605 -4.686314
## 4   0.8012281 0.8012281 -5.125448
## 5   0.7373272 0.7373272 -4.714354
## 6   0.6857306 0.6857306 -4.707360
```
...
You can extract the same WAIC output for `fit1` we saw above by executing `fit1$criteria$waic`. Here we look at the LOO summary for fit1, instead.
...
```r
model_weights(fit5, fit6, weights = "waic") %>% round(2)
```
...
In this case, most of the weight went to the full model, `fit5`. However, the overall pattern between the WAIC difference and the WAIC weights was uncertainty. Make sure to use good substantive reasoning when comparing models.
...
```r
mw <-
  model_weights(fit7, fit8, fit9, weights = "stacking")
...
```r
model_weights(fit7, fit8, fit9, weights = "loo") %>% 
  round(digits = 2)
```
...
Here the evidence varied by the specific weight. Across both, the model with the Haldane prior (`fit9`) did arguably the best. But the model with the uniform prior (`fit8`) was clearly in the running. Overall, the evidence for one versus another was weak.
...
![](10_files/figure-gfm/unnamed-chunk-82-1.png)<!-- -->
...
```r
p1 <-
  pp_check(fit7, type = "bars", nsamples = 1e3) +
  ggtitle("fit7")

p2 <-
  pp_check(fit8, type = "bars", nsamples = 1e3) +
  ggtitle("fit8")

p3 <-
  pp_check(fit9, type = "bars", nsamples = 1e3) +
  ggtitle("fit9")
...
##  [63] gridExtra_2.3             StanHeaders_2.19.0       
##  [65] loo_2.2.0                 stringi_1.4.5            
