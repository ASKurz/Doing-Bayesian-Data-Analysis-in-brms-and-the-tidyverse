...
```{r, fig.width = 2, fig.height = 4.25, warning = F, message = F}
library(patchwork)
...
f %>% 
  ggplot(aes(x = weight)) +
  geom_hline(yintercept = .5, color = pm[7], size = 1/2, linetype = 2) +
  geom_vline(xintercept = thresholds$weight, color = pm[6], size = 2/5) +
...
```{r, fig.width = 3.75, fig.height = 3.5, warning = F, message = F}
posterior_samples(fit21.5) %>% 
...
```{r, fig.width = 3.5, fig.height = 5.25}
# bracket
p1 <-
  tibble(x = .4,
         y = .25,
...
rm(pm, p1, p2, my_arrow, p3, p4, p5, p6, p7, p8, p9, layout, my_data, p, fit21.1, length, n_iter, nd, f, thresholds, stat_wilke, fit21.2, post, b0, b1, n, d_rare, fit21.3, d_not_rare, fit21.4, s, m, d, b2, fit21.5, different_seed, my_upper, my_diag, my_lower, b3, fit21.6, fit21.7, logistic, positions, p10, fit21.8)
```
