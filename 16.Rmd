...
Good times. Let's extract the posterior draws and save them in a data frame `draws_df`.

```{r}
draws_df <- as_draws_df(fit16.1)
```
...
For the next part, we should look at the structure of the posterior draws, `draws_df`.
...
1. `Score` ~ $N$(`r round(draws_df[1, 1], 3)`, `r round(draws_df[1, 2], 3)`)
2. `Score` ~ $N$(`r round(draws_df[2, 1], 3)`, `r round(draws_df[2, 2], 3)`)
3. `Score` ~ $N$(`r round(draws_df[3, 1], 3)`, `r round(draws_df[3, 2], 3)`)
4. `Score` ~ $N$(`r round(draws_df[4, 1], 3)`, `r round(draws_df[4, 2], 3)`)
5. `Score` ~ $N$(`r round(draws_df[5, 1], 3)`, `r round(draws_df[5, 2], 3)`)
6. `Score` ~ $N$(`r round(draws_df[6, 1], 3)`, `r round(draws_df[6, 2], 3)`)

Each row of `draws_df` yields a full model equation with which we might credibly describe the data--or at least as credibly as we can within the limits of the model. We can give voice to a subset of these credible distributions with our version of the upper right panel of Figure 16.3.
...
I'm not aware of an easy way to use `log10(nu)` instead of `nu` with `brms::pairs()`. However, you can get those plots with `as_draws_df()` and a little wrangling.

```{r, fig.width = 2.5, fig.height = 4, warning = F}
draws_df <- as_draws_df(fit16.2)
...
draws_array <- as_draws_array(fit16.2)

draws_array %>% 
  mcmc_acf(pars = c("b_Intercept", "sigma", "nu"), lags = 35)
```
...
draws_array %>% 
  mcmc_dens_overlay(pars = c("b_Intercept", "sigma", "nu"))
```
...
rm(sequence_length, d, beyonce_palettes, bp, my_data, mean_y, sd_y, stanvars, p1, p2, my_arrow, p3, p4, p5, p6, layout, fit16.1, draws_array, draws_df, rope, text, n_lines, n_draws, mu, fit16.2, p7, fit16.3, h, levels)
```
