...
By default, `head()` returned six rows, each one corresponding to the credible parameter values from a given posterior draw. Following our model equation $\text{Score}_i ~ N(\mu, \sigma)$, we might reformat the first two columns as:
...
Note that after using `expand()`, we have a rather long data frame. Anyway, now we're ready to plot.
...
Dont' do all that. Just use robust Bayes. Just for kicks, we can compare the $\sigma$s with `hypothesis()`, too.
...
rm(sequence_length, d, my_data, mean_y, sd_y, stanvars, fit1, post, rope, n_lines, n_draws, mu, p1, p2, fit3)
