...
Just like Kruschke pointed out in the text, the interval for the interaction estimate was much larger than the intervals for the simple contrasts.
...
`r emo::ji("warning")` **Warning**: this one took several hours fit. `r emo::ji("warning")`
...
For a more robust model comparison, we might follow the `kfold()` suggestion in the warning. However, be warned: The `kfold()` function entails refitting the model `K` times. `fit20.1` took about an hour to fit and `fit20.2` took several more. `r emo::ji("warning")` Proceed with caution. `r emo::ji("warning")`
