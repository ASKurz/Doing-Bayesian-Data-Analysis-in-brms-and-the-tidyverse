...
>
> $$p \text{ value} = p(D_{\theta, I} \succcurlyeq D_\text{actual} \mid \theta, I)$$
>
...
d <- crossing(N = factor(level, levels = level),
...
### With intention to fix $N$ {#sec-With-intention-to-fix-N}
...
>
> $$ p(z \mid N, \theta) = \begin{pmatrix} N \\ z \end{pmatrix} \theta^z (1 - \theta)^{N - z}$$
...
flips |> 
  filter(z == 7) |> 
  summarise(`probability in a % metric` = (100 * `p(z/N)`) |> round(digits = 3))
```
...
We got the formula for that last variable, `p(N|z,theta)`, from Formula 11.6 on page 306. You'll note how Kruschke continued to refer to it as $p(z \mid N)$ in his Figure 11.4. It's entirely opaque, to me, how $p(z \mid N) = p(N \mid z, \theta)$. I'm just going with it.
...
flips <- tibble(iter = 1:n_iter,
                n    = rpois(n = n_iter, lambda = 24)) |> 
  mutate(z = map(.x = n, .f = \(n) seq(from = 0, to = n, by = 1))) |> 
  unnest(z) |> 
  mutate(n_choose_z = n_choose_z(n, z)) |> 
  mutate(`p(z/N)` = n_choose_z / sum(n_choose_z),
         `Sample Proportion z/N` = z / n) 
...
flips |>
  ggplot(aes(x = `Sample Proportion z/N`, y = `p(z/N)`,
             color = z <= 7, fill = z <= 7)) +
  geom_col(linewidth = 1/15, width = 0.003) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
  scale_fill_manual(values = jj[1:2]) +
  scale_color_manual(values = jj[1:2]) +
...
If you recall from @sec-Inferring-a-Binomial-Probability-via-Exact-Mathematical-Analysis, we need a function to compute the Bernoulli likelihood.
...
There are a handful of steps before we can use our `bernoulli_likelihood()` function to make the plot data. All these are repeats from @sec-Inferring-a-Binomial-Probability-via-Exact-Mathematical-Analysis.

```{r}
# We need these to compute the likelihood
n <- 24; z <- 7                                                        # (i.e., data)

d_nail <- tibble(theta = seq(from = 0, to = 1, length.out = 1000)) |>  # (i.e., theta)
  mutate(Prior = dbeta(x = theta, shape1 = 2, shape2 = 20)) |> 
  mutate(Likelihood = dbern(x = theta, z = z, n = n)) |>               # (i.e., p(D | theta))
...
If we'd like the 95% HDIs, we'll need to redefine the `hdi_of_icdf()` function. Note the default setting for the `name` argument.
...
d_coin <- tibble(theta = seq(from = 0, to = 1, length.out = 1000)) |>
...
p1 <- tibble(y      = factor(c("tail", "head"), levels = c("tail", "head")),
             `p(y)` = c(1 - 0.126, 0.126)) |> 
  ggplot(aes(x = y, y = `p(y)`)) +
  geom_col(fill = jj[1]) +
  labs(title = "Hypothetical Population",
...
p2 <- tibble(y      = factor(c("tail", "head"), levels = c("tail", "head")),
             `p(y)` = c(1 - 0.511, 0.511)) |> 
  ggplot(aes(x = y, y = `p(y)`)) +
  geom_col(fill = jj[1]) +
  labs(title = "Hypothetical Population",
...
(p1 / p2) &
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) &
  scale_fill_manual(values = jj[1:2]) &
  labs(x = "Sample Proportion z/N",
       y = "p(z/N)",
       title = "Implied Sampling Distribution") &
...
p5 <- tibble(x = 1.5,
  ...
  geom_text(aes(size = size), 
            parse = TRUE, hjust = 0.5, show.legend = FALSE) +
...
## Multiple comparisons {#sec-Multiple-comparisons}
...
d_text <- tibble(
  c           = c(85, 75, 70),
  label       = c("no false alarms", "at least one false alarm",
...
tibble(c = 1:100) |> 
  mutate(p1 =     (1 - alpha_pc)^c,
         p2 = 1 - (1 - alpha_pc)^c) |> 
  pivot_longer(cols = -c, values_to = "probability") |> 
...
"Sampling distributions tell us the probability of imaginary outcomes given a parameter value and an intention, $p(D_{\theta, I} \mid \theta, I)$, instead of the probability of parameter values given the actual data, $(\theta \mid D_\text{actual})$."
