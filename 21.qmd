# Dichotomous Predicted Variable {#sec-Dichotomous-Predicted-Variable}
...
The 3D wireframe plot in Figure 21.1 is technically beyond the scope of our current **ggplot2** paradigm. But we will discuss an alternative in the end of @sec-Example-Height-weight-and-gender.
...
### Example: Height, weight, and gender {#sec-Example-Height-weight-and-gender}
...
as_draws_df(fit21.2) |> 
  ...
  summarise_all(.funs = \(x) mean(x) |> round(digits = 3))
```
...
As far as I can tell, Kruschke chose about $n = 200$ for the data in this example. After messing around with correlations for a bit, it seems $\rho_{x_1, x_2} = 0.975$ looks about right. We can simulate multivariate Gaussian data with a particular correlation with the [`MASS::mvrnorm()` function](https://www.rdocumentation.org/packages/MASS/versions/7.3-51.1/topics/mvrnorm). Since we'll be using standardized $x$-variables, we'll need to specify our $n$, the desired correlation matrix, and a vector of means. Then we'll be ready to do the actual simulation with `mvrnorm()`.
...
f <- fitted(fit21.6,
            ...
            scale = "linear") |> 
  data.frame() |> 
  transmute(prob = Estimate |> plogis())
```
...
## Robust logistic regression {#sec-Robust-logistic-regression}
...
#| fig-width: 4
#| fig-height: 3
#| message: false  # See https://github.com/paul-buerkner/brms/issues/1848

conditional_effects(fit21.7) |> 
  plot(points = TRUE,
       line_args = list(color = pm[2], fill = pm[6]),
       point_args = list(alpha = 3/4, color = pm[1]))
```
...
tibble(m_0 = 0,
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  xlab(expression(mu)) +
  facet_wrap(~ key, labeller = label_parsed, ncol = 3, scales = "free_y")
```
...
nd |> 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        ...
        legend.position.inside = c(0.956, 0.8))
```
