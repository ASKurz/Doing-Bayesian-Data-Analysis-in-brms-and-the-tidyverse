...
To implement those equations, we'll first extract the posterior samples. We'll begin with `fit17.1`, the model for which $N = 300$.
...
As in Chapter 16, "set the priors to be extremely broad relative to the data" (p. 487). With our `stanvars` saved, we're ready to fit `fit17.3`.
...
Looks pretty wide, doesn’t it? Hopefully this isn’t a surprise. Recall that this `post` is from `fit17.2`, the posterior based on the $n = 30$ data. With so few data, most predictions from that model are uncertain. But also, 50 inches is way out of the bounds of the data the model was based on, so we should be uncertain in this range.
...
```{r, echo = F}
# remove the objects
rm(n_draw, d, curves, HtWtDataGenerator, standardize, fit17.1, stanvars, fit17.2, post, make_beta_0, make_beta_1, sd_x, sd_y, m_x, m_y, n_lines, rope, beta_0_sigma, beta_1_sigma, fit17.3, nd, my_data, fit17.4, c, f, fit17.5, make_beta_2)
```
