...
c_small %>% 
  ...
  stat_histintervalh(aes(y = 0),
                     ...
                     breaks = 20, slab_size = .3, outline_bars = T) +
  scale_y_continuous(NULL, breaks = NULL) +
...
d %>% 
  ...
  stat_histintervalh(point_interval = mode_hdi, .width = .95,
                     fill = "grey67", slab_color = "grey92",
                     breaks = 20, slab_size = .4, outline_bars = T) +
  scale_y_continuous(NULL, breaks = NULL) +
  xlab(expression(theta)) +
  theme(panel.grid = element_blank()) +
  facet_wrap(~key, scales = "free_y",  labeller = label_parsed)
...
```{r fit_12.02, cache = T, warning = F, message = F}
fit2 <-
  brm(data = my_data, 
      family = binomial,
...
```{r fit_12.03, cache = T, warning = F, message = F}
fit3 <-
  brm(data = my_data, 
      family = binomial,
