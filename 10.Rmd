...
# save("fit1", file = "fits/10_fit1.rda")
# rm(fit1)

load("fits/10_fit1.rda")
```
...
# save("fit2", file = "fits/10_fit2.rda")
# rm(fit2)

load("fits/10_fit2.rda")
```
...
```{r fit3_and_fit4, cache = T, results = "hide", message = F}
fit3 <-
  brm(data = d,
      family = gaussian,
      y ~ 1,
...
```{r ic_fit3_and_fit4, cache = T, warning = F}
...
```{r pp_averaged_fit1_fit2, warning = F, message = F}
nd <- tibble(y = 1)
...
# save("fit5", file = "fits/10_fit5.rda")
# rm(fit5)

load("fits/10_fit5.rda")
```
...
# save("fit6", file = "fits/10_fit6.rda")
# rm(fit6)

load("fits/10_fit6.rda")
```
...
```{r waic_fit5_fit6, warning = F, echo = F}
fit5 <- add_criterion(fit5, criterion = "waic")
fit6 <- add_criterion(fit6, criterion = "waic")

loo_compare(fit5, fit6, criterion = "waic")
```
...
# save(list = c("fit7", "fit8", "fit9"),
#      file = "fits/10_fit7_to_fit9.rda")
# rm(fit7, fit8, fit9)

load("fits/10_fit7_to_fit9.rda")
```
...
Now let's see what happens when we compute their model weights. Here we'll contrast the LOO weights wiht the stackign weights

```{r mw_fit7_through_fit9, cache = T}
model_weights(fit7, fit8, fit9, weights = "loo") %>% 
  round(digits = 2)
model_weights(fit7, fit8, fit9) %>% 
  round(digits = 2)
```
...
```{r, fig.width = 4, fig.height = 6, message = F}
p1 <-
...
```{r, echo = F}
# remove our objects
rm(length, p_d, p_d_1, p_d_2, d, p1, p2, bernoulli_likelihood, n, z, trial_data, omega, kappa, stanvars, fit1, theta, mean_theta, sd_theta, a_post, b_post, one_over_pd, fit2, mw, nd, pp_a, fit3, fit4, my_data, fit5, fit6, hdi_of_icdf, hdi_of_qbeta, fit7, fit8, fit9, p3)
```
