...
```{r loo_fit5, cache = T, warning = F, echo = F}
# hide from view
fit5 <- add_criterion(fit5, criterion = "loo")
```

```{r loo_fit6, cache = T, warning = F, echo = F}
# hide from view
...
```{r loo_compare_fit5_fit6, warning = F, echo = F}
loo_compare(fit5, fit6)
```
...
```{r mw_fit5_fit6, cache = T, warning = F}
...
```{r loo_fit7_through_fit9, cache = T}
...
```{r, echo = F}
# remove our objects
rm(length, p_d, p_d_1, p_d_2, d, p1, p2, bernoulli_likelihood, n, z, trial_data, omega, kappa, stanvars, fit1, theta, mean_theta, sd_theta, a_post, b_post, one_over_pd, fit2, mw, nd, pp_a, fit3, fit4, my_data, fit5, fit6, hdi_of_icdf, hdi_of_qbeta, fit7, fit8, fit9)
```
