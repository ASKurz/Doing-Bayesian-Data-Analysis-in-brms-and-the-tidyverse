...
tibble(omega = seq(from = 0, to = 1, length.out = length)) %>% 
  ...
  scale_x_continuous(expand = expansion(mult = c(0.0025, 0.05)), limits = c(0, 25)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0))) +
  labs(subtitle = "Remember, the scale on the x is arbitrary.",
       x = expression(Marginal~p(omega)),
       y = expression(omega))
```
...
p1 <-
  ...
  scale_x_continuous(expression(theta), breaks = 0:5 / 5, expand = expansion(mult = c(0, 0))) +
  scale_y_continuous(expression(p(theta*"|"*omega==.75)), expand = expansion(mult = c(0, 0.05)))
...
p2 <-
  ...
  geom_ribbon(fill = kh[4]) +
  scale_x_continuous(expression(theta), breaks = 0:5 / 5, expand = expansion(mult = c(0, 0))) +
  scale_y_continuous(expression(p(theta*"|"*omega==.25)), expand = expansion(mult = c(0, 0.05)))
...
d %>% 
  ...
  scale_x_continuous(expression(omega), 
                     expand = expansion(mult = c(0.0025, 0.05)), limits = c(0, 25)) +
...
tibble(x = seq(from = 0, to = 100, by = 1)) %>% 
  ...
  scale_y_continuous(NULL, breaks = NULL,
                     expand = expansion(mult = c(0, 0.05))) +
  scale_x_continuous(expression(italic(p)(nu)), expand = expansion(mult = c(0, 0.01)))
```
...
pp_check(fit10.3, nsamples = 50)
pp_check(fit10.4, nsamples = 50)
```
...
```{r, echo = F}
# remove our objects
rm(d, kh, length, p1, p2, my_arrow, p3, p4, p5, p6, p7, p8, layout, p_d, p_d_1, p_d_2, odds, bernoulli_likelihood, n, z, trial_data, omega, kappa, stanvars, fit10.1, theta, mean_theta, sd_theta, a_post, b_post, one_over_pd, fit10.2, mw, nd, pp_a, fit10.3, fit10.4, my_data, fit9.2, fit10.5, fit10.6, hdi_of_icdf, hdi_of_qbeta, fit10.7, fit10.8)
```
