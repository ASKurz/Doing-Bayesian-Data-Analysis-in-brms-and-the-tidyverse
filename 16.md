...
If we want to pass user-defined values into our brms prior code, we'll need to define them first in using `brms::stanvar()`.
...
We've saved that as `stanvars`, but you could name it whatever you want. But the trick is to them tell `brm()` about your values in a `stanvars` statement. Recall that the Stan team [discourages uniform priors for variance parameters](https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations), such as our *σ*. Here we'll use the half Normal.
...
Each row of `post` yields a full model equation that is a credible description of the data—or at least as credible as we can get within the limits of the model we have specified. We can give voice to a subset of these credible distributions with our version of the upper right panel of Figure 16.3.
...
It's easy to use Student's *t* in brms. Make sure to specify `family = student`. Be default, brms already sets the lower bound for *ν* to 1, so there's no need to worry about shifting the exponential prior. Also, Stan will bark if you try to enter the `1/30` into the exponential prior:

> DIAGNOSTIC(S) FROM PARSER: Warning: integer division implicitly rounds to integer. Found int division: 1 / 30 Positive values rounded down, negative values rounded up or down in platform-dependent way.
...
stanvars <- 
  stanvar(mean_y, name = "mean_y") + 
  stanvar(sd_y, name = "sd_y") + 
  stanvar(1/30, name = "one_over_thirty")
```
...
fit2 <-
  brm(data = my_data,
      ...
      prior = c(prior(normal(mean_y, sd_y*100), class = Intercept),
                prior(normal(0, sd_y), class = sigma),
                prior(exponential(one_over_thirty), class = nu)),
      chains = 4, cores = 4,
      stanvars = stanvars)
```
...
```
    ...
    ## b_Intercept 1.000000000 0.02476258 0.004296776
    ## sigma       0.024762577 1.00000000 0.743756486
    ## log10(nu)   0.004296776 0.74375649 1.000000000
...
```
    ...
    ## Population-Level Effects: 
    ##           Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept   107.23      2.83   101.54   112.75       2523 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma    19.59      3.47    13.18    26.59       1726 1.00
    ## nu        9.53     12.95     1.84    48.24       1610 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
...
![](16_files/figure-markdown_github/unnamed-chunk-32-1.png)
...
```
    ...
    ##   0.9998448   1.0010715   1.0018712   1.0019914

If you peer into the contents of a `brm()` fit object (e.g., `fit2 %>% str()`), you'll see that it contains the Scan code. Here it is for our `fit2`.
...
```
    ...
    ##   target += exponential_lpdf(nu | one_over_thirty)
    ##     - 1 * exponential_lccdf(1 | one_over_thirty); 
...
```
    ...
    ## Population-Level Effects: 
    ##                      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## GroupPlacebo            99.30      1.81    95.78   102.89       4000 1.00
    ## GroupSmartDrug         107.09      2.57   101.97   112.02       4000 1.00
    ## sigma_GroupPlacebo       2.38      0.15     2.08     2.67       3278 1.00
    ## sigma_GroupSmartDrug     2.84      0.15     2.54     3.14       3432 1.00
    ## 
    ## Family Specific Parameters: 
    ##    Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## nu     3.58      1.45     1.83     7.04       2235 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
...
```
    ...
    ## Observations: 4,000
    ## Variables: 6
    ## $ b_GroupPlacebo         <dbl> 99.24441, 98.53356, 98.97867, 97.92235,...
    ## $ b_GroupSmartDrug       <dbl> 106.9147, 104.7450, 104.3373, 106.1249,...
    ## $ b_sigma_GroupPlacebo   <dbl> 2.443559, 2.518625, 2.326085, 2.512222,...
    ## $ b_sigma_GroupSmartDrug <dbl> 2.887436, 2.877697, 2.816490, 2.957079,...
    ## $ nu                     <dbl> 4.444047, 5.289001, 3.253360, 4.309875,...
    ## $ lp__                   <dbl> -548.5492, -549.7495, -548.8376, -549.1...

We may as well clean convert the columns a bit to make their names and metrics reflect those in the text.
...
```
    ...
    ## Observations: 4,000
    ## Variables: 8
    ## $ `Placebo Mean`         <dbl> 99.24441, 98.53356, 98.97867, 97.92235,...
    ## $ `Smart Drug Mean`      <dbl> 106.9147, 104.7450, 104.3373, 106.1249,...
    ## $ `Placebo Scale`        <dbl> 11.513941, 12.411522, 10.237779, 12.332...
    ## $ `Smart Drug Scale`     <dbl> 17.94723, 17.77329, 16.71806, 19.24168,...
    ## $ Normality              <dbl> 0.6477786, 0.7233736, 0.5123322, 0.6344...
    ## $ `Difference of Means`  <dbl> 7.670292, 6.211407, 5.358670, 8.202528,...
    ## $ `Difference of Scales` <dbl> 6.4332869, 5.3617655, 6.4802822, 6.9093...
    ## $ `Effect Size`          <dbl> 0.5087178, 0.4052153, 0.3865748, 0.5075...
...
![](16_files/figure-markdown_github/unnamed-chunk-45-1.png)
...
```
    ...
    ##  [1] bayesplot_1.5.0       gridExtra_2.3         tidybayes_0.12.1.9000
    ##  [4] brms_2.4.0            Rcpp_0.12.18          bindrcpp_0.2.2       
