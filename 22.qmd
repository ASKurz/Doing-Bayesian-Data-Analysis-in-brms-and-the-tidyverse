# Nominal Predicted Variable {#sec-Nominal-Predicted-Variable}
...
## Softmax regression {#sec-Softmax-regression}
...
Let's load the primary packates for the chapter.
...
Before we explore these data in a plot, let's talk color and theme. For this chapter, we'll carry forward our practice from @sec-Dichotomous-Predicted-Variable and take our color palette from the [**PNWColors** package](https://CRAN.R-project.org/package=PNWColors). This time, our color palette will be `"Lake"`.
...
## Conditional logistic regression {#sec-Conditional-logistic-regression}
...
tibble(
  ...
  geom_text(data = d_text,
            aes(label = label),
            family = "Times", size = 5) +
  geom_text(data = d_greek,
            aes(label = label, hjust = hjust),
            family = "Times", parse = TRUE, size = 4.25) +
  xlim(-1, 7) +
  theme_void()
```
...
$$
\begin{align*}
\lambda_{\{ 1 \} \mid \{ 1,2,3,4 \}} & = \beta_{0,\{ 1 \} \mid \{ 1,2,3,4 \}} + \beta_{1,\{ 1 \} \mid \{ 1,2,3,4 \}} \text{X1} + \beta_{2,\{ 1 \} \mid \{ 1,2,3,4 \}} \text{X2} \\
\lambda_{\{ 2 \} \mid \{ 2,3,4 \}}   & = \beta_{0,\{ 2 \} \mid \{ 2,3,4 \}} + \beta_{1,\{ 2 \} \mid \{ 2,3,4 \}} \text{X1} + \beta_{2,\{ 2 \} \mid \{ 2,3,4 \}} \text{X2} \\
...
crossing(X1 = seq(from = -2, to = 2, length.out = 50),
  ...
  # Wrangle
  pivot_longer(cols = contains("phi"), values_to = "phi") |> 
  
  # Plot
  ggplot(aes(x = X1, y = X2)) +
  geom_raster(aes(fill = phi),
              interpolate = TRUE) +
  # Note how we're subsetting the data by facet
...
p1 <- tibble(x = seq(from = -3, to = 3, by = 0.1)) |> 
  ...
  geom_area(color = pl[5], fill = pl[6]) +
  annotate(geom = "text",
           x = 0, y = 0.2,
           label = "normal",
           color = pl[1], size = 7) +
  annotate(geom = "text",
           ...
           color = pl[1], family = "Times", hjust = 0, parse = TRUE, size = 7) +
  scale_x_continuous(expand = c(0, 0)) +
  theme_void() +
  theme(axis.line.x = element_line(color = pl[1], linewidth = 0.5),
        plot.background = element_rect(color = "white", fill = pl[8], linewidth = 1))
...
p3 <- tibble(x = 0.5,
  ...
  annotate(geom = "text",
           x = c(0.48, 0.72), y = 0.5,
           label = c("'~'", "italic(k)"),
           color = pl[1], family = "Times", parse = TRUE, size = c(10, 7)) +
  xlim(0, 1) +
  theme_void()
...
p5 <- tibble(x = 0.5,
  ...
  geom_text(color = pl[1], family = "Times", parse = TRUE, size = 7) +
...
p6 <- tibble(x = c(0.375, 0.6),
             ...
             label = c("'='", "italic(i)")) |> 
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(color = pl[1], family = "Times", parse = TRUE, size = c(10, 7)) +
...
p7 <- tibble(x = 0:3,
  ...
  ggplot(aes(x = x, y = d)) +
  geom_col(color = pl[5], fill = pl[6], width = 0.45) +
  annotate(geom = "text",
           x = 1.5, y = 0.2,
           label = "categorical",
           color = pl[1], size = 7) +
  annotate(geom = "text",
           ...
           color = pl[1], family = "Times", parse = TRUE, size = 7) +
...
p3 <- tibble(x = 0.5,
  ...
  annotate(geom = "text",
           x = 0.49, y = 0.5,
           label = "'~'",
           color = pl[1], family = "Times", parse = TRUE, size = 10) +
  xlim(0, 1) +
  theme_void()
...
p7 <- tibble(x = 0:1,
  ...
  ggplot(aes(x = x, y = d)) +
  geom_col(color = pl[5], fill = pl[6], width = 0.4) +
  annotate(geom = "text",
           x = 0.5, y = 0.2,
           label = "Bernoulli",
           color = pl[1], size = 7) +
...
As indicated in the formulas in @sec-Softmax-regression, we get posteriors for each level of `Y`, except for `Y == 1`. That serves as the reference category. The values for $\beta_{i, k = 1}$ are all fixed at $0$.
...
draws |> 
  ...
  mutate(lambda    = str_c("lambda==", str_extract(name, "[2-4]+")),
  ...
  stat_histinterval(point_interval = mode_hdi, .width = 0.95,
                    color = pl[1], fill = pl[4], normalize = "panels", 
                    point_color = pl[2], slab_color = pl[3]) +
...
fitted(fit22.1,
  ...
  bind_cols(nd) |> 
  pivot_longer(cols = contains("Y"), values_to = "phi") |> 
...
##### Bonus: Consider the interceps-only softmax model {#sec-Bonus-Consider-the-interceps-only-softmax-model}
...
p1 <- d1 |> 
  ...
  ggplot(aes(x = Y, y = p)) +
  geom_col(fill = pl[6]) +
  scale_y_continuous(NULL, breaks = 0:4 / 10, expand = expansion(mult = c(0, 0.05)), 
...
p1 + p2 + 
  plot_annotation(title = "The softmax model adds information to the conventional sample-based\nbar plot.")
```
...
```{r}
# For annotation
d_text <- fitted(fit22.2)[1, , ] |> 
  t() |> 
  data.frame() |> 
  rownames_to_column("level") |> 
  mutate(y       = (Estimate / 2) + lag(cumsum(Estimate), default = 0),
         label   = str_c("Y = ", 1:4),
         summary = str_c(round(100 * Estimate, 1), "*'% ± '*", round(200 * Est.Error, digits = 1)))

p4 <- fitted(fit22.2, summary = FALSE)[, 1, ] |> 
  data.frame() |> 
  set_names(1:4) |> 
  mutate(row = 1:n()) |> 
  ...
  geom_col(aes(fill = Y),
           linewidth = 0, position = "stack", width = 1) +
  geom_text(data = d_text,
            aes(x = -2000, y = y, label = label),
            color = pl[1]) +
  geom_text(data = d_text,
            aes(x = 6000, y = y, label = summary),
            color = pl[1], parse = TRUE) +
  scale_x_continuous(NULL, breaks = NULL, limits = c(-4000, 8000)) +
  scale_y_continuous(NULL, expand = c(0, 0), labels = scales::percent) +
  scale_fill_manual(values = pl[7:4], breaks = NULL) +
  labs(subtitle = "stacked bar plot with uncertain boundaries")
```

Okay, now combine the four panels, entitle, and behold!
...
As we briefly learned in @sec-Defining-new-likelihood-functions, **brms** users can define their own custom likelihood functions, which Bürkner outlined in his [-@Bürkner2022Define] vignette, [*Define custom response distributions with brms*](https://CRAN.R-project.org/package=brms/vignettes/brms_customfamilies.html). As part of the [*Nominal data and Kruschke's "conditional logistic" approach*](https://discourse.mc-stan.org/t/nominal-data-and-kruschkes-conditional-logistic-approach/21433) thread on the Stan forums, Henrik Singmann showed how you can use this functionality to fit conditional logistic models with **brms**. We will practice how to do this for the models of both the `d3` and `d4` data sets, which were showcased in the left and right panels of Figure 22.3 in @sec-Conditional-logistic-regression. Going in order, we'll focus first on how to model the data in `d3`.
...
* the lines with no prefix have to do with $\lambda_{\{ 1 \} \mid \{ 1,2,3,4 \}}$,
* `mub_` has to do with $\lambda_{\{ 2 \} \mid \{ 2,3,4 \}}$, and
* `muc_` has to do with $\lambda_{\{ 3 \} \mid \{ 3,4 \}}$.
...
# Wrangle
p1 <- draws |> 
  pivot_longer(cols = a_Intercept:c_X2) |> 
...
p2 <- draws |> 
  ...
  geom_abline(aes(intercept = intercept,
                  ...
                  linetype = mu),
              alpha = 1/2, color = pl[1], linewidth = 1/4) +
  scale_color_manual(values = pl[2:5]) +
  scale_linetype(NULL,
                 labels = parse(text = c(
                   "lambda['{1}|{1,2,3,4}']", 
...
bind_rows(
  ...
  pivot_longer(cols = starts_with("ess")) |> 
...
In their [-@burknerOrdinalRegressionModels2019] paper, [*Ordinal regression models in psychology: A tutorial*](https://psyarxiv.com/x8swp/), Bürkner and Vourre outlined a framework for fitting a variety of original models with **brms**. We'll learn more about ordinal models in @sec-Ordinal-Predicted-Variable. In this section, we'll use Mattan Ben-Shachar's strategy and purpose one of the ordinal models to fit a conditional logistic model to our nominal data.
...
rbind(fixef(fit22.3)[c(1:4, 6, 8, 5, 7, 9), ], fixef(fit22.5)) |> 
  ...
  geom_vline(xintercept = 0, color = "white") +
  geom_pointrange(fatten = 3/2, linewidth = 1/2) +
...
These models can be generalized to include different kinds of predictors, variants robust to outliers, and model comparison via information criteria and so forth. You can find a few more examples with softmax regression in Chapter 10 of the first edition of McElreath's [-@mcelreathStatisticalRethinkingBayesian2015] [*Statistical rethinking*](https://xcelab.net/rm/statistical-rethinking/) and Chapter 11 of his second edition [@mcelreathStatisticalRethinkingBayesian2020]. See also Kurz [-@kurzStatisticalRethinkingSecondEd2023, [Section 11.3](https://bookdown.org/content/4857/god-spiked-the-integers.html#multinomial-and-categorical-models)] and Kurz [-@kurzStatisticalRethinkingBrms2023, [Section 10.3.1](https://bookdown.org/content/3890/counting-and-classification.html#multinomial)] for walk-throughs with **brms**.
