...
# too large and slow to fit multiple times

# save("fit1", file = "/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit1.rda")
# rm(fit1)

load("/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit1.rda")
```
...
This was a difficult model to fit with **brms**. Stan does well when the criteria are on or close to a standardized metric. These `Salary` data are a far far cry from that. Tuning `adapt_delta` and `max_treedepth` helped a bit, but the effective samples are still modest given the number of iterations.
...
```{r, echo = F, warning = F, message = F}
# too large and slow to fit multiple times
# save("fit2", file = "/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit2.rda")
# rm(fit2)

load("/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit2.rda")
```
...
For a more robust model comparison, we might follow the `kfold()` suggestion in the warning. However, be warned: The `kfold()` function entails refitting the model `K` times. Both these models took hours to fit, particularly `fit2`.
...
```{r, echo = F, warning = F, message = F}
# especially with that reloo below, too large and slow to fit multiple times
# save("fit3", file = "/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit3.rda")
# rm(fit3)

load("/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit3.rda")
```
...
```{r, echo = F, warning = F, message = F}
# too large and slow to fit multiple times
# save("fit4", file = "/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit4.rda")
# rm(fit4)

load("/Users/solomon/Dropbox/Recoding Doing Bayesian Data Analysis/fits/20_fit4.rda")
```
...
> Found 5 observations with a pareto_k > 0.7 in model 'fit3'. It is recommended to set 'reloo = TRUE' in order to calculate the ELPD without the assumption that these observations are negligible. This will refit the model 5 times to compute the ELPDs for the problematic observations directly.
...
```{r, message = F, echo = F}
# remove the objects
rm(grand_mean, deflection_1, deflection_2, nonadditive_component, d, p, p1, p2, p3, gamma_a_b_from_omega_sigma, my_data, mean_y, sd_y, omega, sigma, s_r, stanvars, n_draw, f, n_iter, post, nd, brief_col_names, f1, text, f2, fit3, col_names, fit4)
```
