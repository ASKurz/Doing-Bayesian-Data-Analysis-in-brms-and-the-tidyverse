...
tibble(theta = seq(from = 0, to = 1, by = 0.001)) |> 
  ...
  labs(x = expression(theta),
       subtitle = expression("Behold our Beta"*(1299.7*', '*700.3)*" prior. It's rather peaked"))
```
...
And indeed our samples did tend around $\theta = 0.643$. Though he presented Table 13.1 in this section, Kruschke walked out how he came to those values in the following sections. We'll get to them in just a bit.
...
tibble(theta = seq(from = 0, to = 1, by = 0.01)) |> 
  ...
  labs(x = expression(theta),
       title = "Behold the uniform audience prior.")
```
...
sim <- crossing(mode  = seq(from = 0.6, to = 0.85, by = 0.05),
                power = c(0.7, 0.8, 0.9)) |> 
  mutate(results = map2_dbl(.x = mode, .y = power, .f = sim_power))
```
...
You may (or not) recall that we covered how to time an operation in **R** back in @sec-Measuring-processing-time. When you're setting up a Monte Carlo power study, it can be important to use those time-tracking skills to get a sense of how long it takes to fit your models. While I was setting this model up, I experimented with keeping the default `cores = 1` or setting my typical `cores = 4`. As it turns out, with a very simple model like this, `cores = 1` was a little faster. If you're fitting one model, that's no big deal. But in a situation where you're fitting 100 or 1,000, you'll want to make sure you're fitting them as efficiently as possible.
...
sim1 <- tibble(n     = n,
  ...
  mutate(fit = map(.x = data, 
                   .f = \(data) update(fit13.1, newdata = list(y = data))))
```
...
With `sim2`, we avoided saving our model `brms::brm()` fit objects by using the line:
...
sim3 <- tibble(seed = 1:n_sim) |> 
  ...
  mutate(hdi = map2(.x = data, .y = seed, .f = fit_then_hdis))
```
