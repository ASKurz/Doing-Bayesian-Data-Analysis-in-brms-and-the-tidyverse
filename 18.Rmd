...
If $y ~ N(\mu, \sigma)$ and $\mu = \beta_0 + \beta_1 x_1 + \beta_2 x_2$, then it's also the case that we can rewrite the formula for $y$ as:

$$y ~ N(\beta_0 + \beta_1 x_1 + \beta_2 x_2, \sigma)$$
...
The `brms::bayes_R2()` function makes it easy to compute a Bayesian $R^2$. Simply feed a model brm fit object into `bayes_R2()` and you'll bet back the posterior mean, $SD$, and 95% intervals.

```{r}
bayes_R2(fit1)
```

I'm not going to go into the technical details here, but you should be aware that the Bayeisan $R^2$ is not calculated the same as it is with OLS. If you want to dive in, check out the paper by [Gelman, Goodrich, Gabry, and Ali](https://github.com/jgabry/bayes_R2/blob/master/bayes_R2.pdf). Anyway, if you'd like to view the Bayesian $R^2$ distribution rather than just get the summaries, specify `summary = F`, convert the output to a tibble, and plot as usual.
...
Kruschke mentioned high autocorrelations in the prose. Here are the autocorrelation plots for our $\beta$s.
...
So it turns out the correlations among the redundant predictors were still very high. You can really get a sense of the silliness of the parameters if you plot them. We'll use `geom_halfeyeh()` to get a sense of densities and summaries of the $\beta$s.
...
$$= \beta_0 + (\beta_1 + \beta_{1 \times 2} x_2) x_1 + \beta_2 x_2$$

$$= \beta_0 + \beta_1 x_1 + (\beta_2 + \beta_{1 \times 2} x_1) x_2$$
...
where $\delta$ is a dummy for which 0 = *not included* 1 = *included*. I'm not aware of a way to use $\delta$ as an inclusion indicator in brms the way Kruschke implemented it in JAGS. If you know of a way [share your code](https://github.com/ASKurz/Doing-Bayesian-Data-Analysis-in-brms-and-the-tidyverse/issues). However, this issue lead to a similar approach: information criteria, about which you can learn more about [here](https://www.youtube.com/watch?v=t0pRuy1_190&t=1331s&frags=pl%2Cwn). To do so, let's follow Kruschke's basic flow and use the first model from way back in subsection 18.1.1 as a starting point. The model formula was as follows.

```{r}
fit1$formula
```

Taking interactions off the table for a moment, we can specify for model types with various combinations of the two predictors, `PrcntTake_z` and `Spend_z`. `fit1` was the first, which we might denote as $\langle 1, 1 \rangle$. That leads to the remaining possibilities as
...
Let's fit the models with the updated $SD = 1$ on the $\beta_{1+}$ priors code.

```{r fit9_through_12, cache = T, message = F, warning = F}
fit9 <-
  update(fit1,
...
```{r fit13_through_16, cache = T, message = F, warning = F}
fit13 <-
  update(fit9,
...
So unlike in the depictions in Figure 18.14, the stacking method was fairly insensitive to the $SD$s on our $\beta_j$ priors. We might compare LOO difference scores, too.
...
```{r mw_fit_1111_to_fit_0000, cache = T, message = F, warning = F}
(
  mw <-
...
rm(n, d, mus, sds, cors, covs, my_data, standardize, stanvars, fit1, post, make_beta_0, make_beta_j, sd_x_1, sd_x_2, sd_y, m_x_1, m_x_2, m_y, percent_take, fit2, sd_x_3, m_x_3, prior, fit3, spend_slope, percent_take_slope, fit4, sd_x_4, sd_x_5, sd_x_6, sd_x_7, sd_x_8, sd_x_9, sd_x_10, sd_x_11, sd_x_12, sd_x_13, sd_x_14, m_x_4, m_x_5, m_x_6, m_x_7, m_x_8, m_x_9, m_x_10, m_x_11, m_x_12, m_x_13, m_x_13, fit5, fit6, fit7, fit8, l_fit1, l_fit6, l_fit7, l_fit8, mw, fit9, fit10, fit11, fit12, fit13, fit14, fit15, fit16, gamma_s_and_r_from_mode_sd, fit_1111, fit_0111, fit_1011, fit_1101, fit_1110, fit_0011, fit_0101, fit_0110, fit_1001, fit_1010, fit_1100, fit_0001, fit_0010, fit_0100, fit_1000, fit_0000, n_points, nd, pp)
