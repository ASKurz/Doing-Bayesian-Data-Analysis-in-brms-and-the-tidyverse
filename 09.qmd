...
On page 224, Kruschke wrote: "The value of $\kappa$ governs how near $\theta$ is to $\omega$, with larger values of $\kappa$ generating values of $\theta$ more concentrated near $\omega$." To give a sense of that, we'll simulate 20 beta distributions, all with $\omega = 0.25$ but with $\theta$ increasing from 10 to 200, by 10. We'll then plot them with a little help from the [**ggridges** package](https://CRAN.R-project.org/package=ggridges) [@R-ggridges].
...
p2 <- tibble(
  x = c(0.5, 0.475, 0.26, 0.08, 0.06, 0.5, 0.55, 0.85, 1.15, 1.2),
...
d <- crossing(theta = parameter_space,
...
d_text <- tibble(theta = c(0.75, 0.25),
                 y     = 10,
...
```{r}
d <- crossing(theta_1 = parameter_space,
...
Before we make the plots in the middle row of Figure 9.5, we need to add the likelihoods. Recall that we're presuming the coin flips contained in $D_1$ and $D_2$ are independent. Kruschke explained in @sec-Prior-likelihood-and-posterior-for-two-biases, that
...
$$p(D \mid \theta_1, \theta_2) = \left ( \theta_1^{z_1} (1 - \theta_1) ^ {N_1 - z_1} \right ) \left ( \theta_2^{z_2} (1 - \theta_2) ^ {N_2 - z_2} \right ).$$
...
### A realistic model with MCMC {#sec-A-realistic-model-with-MCMC}
...
# A gamma density
p2 <- tibble(x = seq(from = 0, to = 5, by = 0.01),
...
p3 <- tibble(
  x = c(0.5, 0.475, 0.26, 0.08, 0.06, 0.5, 0.55, 0.85, 1.15, 1.175,
...
### Example: Therapeutic touch {#sec-Example-Therapeutic-touch}
...
But anyway, we'll be doing logistic regression using the logit link. Kruschke covered this in detail in @sec-Dichotomous-Predicted-Variable.
...
# Wrangle
draws_small <- draws |> 
  # Convert the linear model parameters to the probability space with `plogis()`
...
sigmas <- coef(fit9.1, summary = FALSE)$s |> 
  as_tibble() |> 
...
See how we're using `proc.time()` to record when we began and finished evaluating our `brm()` code? The last time we covered that was way back in @sec-Measuring-processing-time. In that section, we also learned how subtracting the former from the latter yields the total elapsed time.
...
# Half-normal density
p1 <- tibble(x = seq(from = 0, to = 3, by = 0.01)) |> 
...
### Example: Baseball batting abilities by position {#sec-Example-Baseball-batting-abilities-by-position}
...
```{r}
coef_small <- coef_primary_position |> 
...
```{r}
p1 <- fitted_positions |>  
  pivot_longer(cols = everything(), values_to = "theta") |> 
