...
$$\text{beta} (\alpha = \omega (\kappa - 2) + 1, \beta = (1 - \omega) \cdot (\kappa - 2) + 1),$$
...
Note how we saved each the plots as objects. If we want to stack the plots one atop another like they're presented in Figure 6.5, we can use the `grid.arrange()` function from the [**gridExtra** package](https://cran.r-project.org/web/packages/gridExtra/index.html).
...
```{r, echo = F, eval = F}
rm(bernoulli_likelihood, betaABfromMeanKappa, betaABfromModeKappa, betaABfromMeanSD, beta_param, hdi_of_icdf, n, z, a, b, proportion_heads, prior_mean, posterior_mean, trial_data, d, d_line, d_text, get_hdi, theta, p_theta, BernGrid, p1, p2, p3)
```
