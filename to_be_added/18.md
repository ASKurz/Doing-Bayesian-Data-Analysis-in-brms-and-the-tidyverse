...
Like we did in Chapter 17, let’s wrap the consequences of equation 18.1
into two
functions.
...
That worked like a charm. We’ll follow the same basic order of
...
\(SD\)s on our \(\beta_j\) priors. Let’s finish up with the histograms
comparing the model predictors. Here’s the code for those in the top
portion of Figure 18.14.
...
`fit17` through `fit33`, we used Kruschke’s \(\delta\) 0/1 convention.
If we set the formula for the full model as `satt_z ~ 1 + spend_z +
prcnt_take_z + stu_tea_rat_z + salary_z`, the name becomes `fit_1111`.
Accordingly, we called the model omitting `spend_z`, the first
predictor, `fit_0111`, and so on.

Before we go any further, here are the correlations among the \(\beta\)s
for the full model, `fit_1111`.
...
Figure 18.15. Here we’ll plot the histograms for our top
six.
...
posterior_samples(fit_0100) %>% 
  ...
  # fit_1110       # spend_z + prcnt_take_z + stu_tea_rat_z
  bind_rows(
    posterior_samples(fit_1110) %>% 
      transmute(Spend     = make_beta_j(zeta_j = b_spend_z,
                                        sd_j   = sd_x_1,
                                        sd_y   = sd_y),
                PrcntTake = make_beta_j(zeta_j = b_prcnt_take_z,
  ...
  mutate(key = factor(key, levels = c("Spend", "PrcntTake", "StuTeaRat", "Salary")),
         fit = factor(fit, levels = c("fit_0100", "fit_0111", "fit_1100", "fit_0001", "fit_1000", "fit_1110"))) %>%
...
Like Kruschke’s results in the text, `PrcntTake` was the most prominant
predictor.
...
```
    ...
    ##  [15] bridgesampling_0.7-2      mnormt_1.5-5             
    ...
    ##  [95] readxl_1.3.1              callr_3.3.2              
    ##  [97] threejs_0.3.1             digest_0.6.21            
