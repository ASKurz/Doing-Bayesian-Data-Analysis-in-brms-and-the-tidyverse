...
$$
\\begin{eqnarray}
p (\\hat y | D) & = & \\sum\_m p (\\hat y | D, m) p (m | D) \\\\
& = & \\sum\_m \\int \\text d \\theta\_m p\_m (\\hat{y} | \\theta\_m, m) p\_m(\\theta\_m | D, m) p (m | D)
\\end{eqnarray}
$$
...
"The full model has a distinct modal batting ability, *ω*<sub>*c*</sub> , for each of the nine fielding positions. The full model also has distinct concentration parameters for each of the nine positions" (p. 291). Let's fit that model again.
...
Recall that when we combine a Beta(*θ*|*α*, *β*) prior with the results of a Bernoulli likelihood, we get a posterior defined by Beta(*θ*|*z* + *α*, *N* − *z* + *β*).
...
```
    ...
    ##         fit7         fit8         fit9 
    ## 1.117677e-01 8.881542e-01 7.810013e-05
...
Like we did earlier with `fit1` and `fit2`, we can use the `pp_average()` function to compute the stacking weighted posterior for *θ*.
...
![](10_files/figure-markdown_github/unnamed-chunk-72-2.png)

``` r
pp_check(fit9, type = "bars", nsamples = 1e3)
```

![](10_files/figure-markdown_github/unnamed-chunk-72-3.png)
