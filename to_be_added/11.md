...
```
    ...
    ##  [1] "H" "T" "T" "H" "T" "T" "H" "H" "T" "H" "H" "H" "T" "H" "T" "T" "H"
    ## [18] "H" "T" "H" "H" "T" "T" "T"

I wanted the solution to be reproducible, which required I find the
...
```
    ...
    ## # A tibble: 0 x 2
    ## # … with 2 variables: seed <int>, n_heads <int>
...
```
    ...
    ##  [1] "H" "T" "H" "T" "H" "H" "T" "H" "H" "H" "H" "T" "H" "T" "H" "T" "H"
    ## [18] "H" "H" "H" "T" "H" "T" "T"
...
To do factorials in R, use the `factorial()` function. E.g., we can use
the formula near the bottom of the main paragraph on page 303 like so:
...
That formula’s a little cumbersome. We can make our programming lives
easier by wrapping it into a function.
...
Now we can employ `N_choose_z()` to help make the data we’ll use for
Figure 11.3.b. Here are the data.
...
We can get the one-sided \(p\)-value with a quick `filter()` and
`summarise()`.
...
Here’s a glance at the Poisson distribution for which \(\lambda = 24\).
...
Here’s my attempt to make a mixture of binomial distributions. I use the
custom `z_maker()` function to simulate \(N\)-values from
\(Poisson(24)\). We then plug in sequences of 0:\(N\) into a tibble and
compute `p(z/N)` and such, by sequence.
...
I played around with the simulation a bit, and this is about as good as
...
![](11_files/figure-gfm/unnamed-chunk-17-1.png)<!-- -->

If you know how to do the simulation properly, [please share your
code](https://github.com/ASKurz/Doing-Bayesian-Data-Analysis-in-brms-and-the-tidyverse/issues).
...
Kruschke began the subsection with “The Bayesian interpretation of data
does not depend on the covert sampling and testing intentions of the
data collector.”
...
```
    ...
    ## $ Prior                <dbl> 0.0000000, 0.4124961, 0.8094267, 1.1912097,…
    ## $ Likelihood           <dbl> 0.000000e+00, 9.900280e-22, 1.245822e-19, 2…
    ## $ normalizing_constant <dbl> 5.266333e-08, 5.266333e-08, 5.266333e-08, 5…
    ## $ Posterior            <dbl> 0.000000e+00, 7.754592e-15, 1.914808e-12, 4…
...
![](11_files/figure-gfm/unnamed-chunk-20-1.png)<!-- -->

To get the left column of Figure 11.7, we have to update the data with
our new prior, \(\text{beta} (11, 11)\).
...
![](11_files/figure-gfm/unnamed-chunk-24-1.png)<!-- -->
...
The upper left panel in Figure 11.9 is the same as 11.8, which you can
...
![](11_files/figure-gfm/unnamed-chunk-26-1.png)<!-- -->
...
![](11_files/figure-gfm/unnamed-chunk-27-1.png)<!-- -->
...
Here’s the lower left panel for Figure
11.10.
...
![](11_files/figure-gfm/unnamed-chunk-29-1.png)<!-- -->
...
Let’s leave failure behind. Here’s the upper left panel for Figure
11.11.
...
```
    ...
    ##  [1] gridExtra_2.3   forcats_0.4.0   stringr_1.4.0   dplyr_0.8.1    
    ...
    ##  [7] vctrs_0.1.0       htmltools_0.3.6   viridisLite_0.3.0
