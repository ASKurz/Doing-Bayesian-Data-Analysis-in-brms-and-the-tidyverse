...
Our `hdi_of_icdf()` function will compute the analytic 95% highest density intervals (HDIs) for the distribution under consideration in Figure 7.1, $\operatorname{Beta}(\theta \mid 15, 7)$.
...
Using an equation from @sec-Specifying-a-beta-prior, $\omega = (a − 1) / (a + b − 2)$, we can compute the corresponding mode.
...
tibble(theta = seq(from = 0, to = 1, length.out = 101)) |> 
  ...
  annotate(geom = "pointrange",
           x = omega, xmin = h[1], xmax = h[2], y = 0, 
           size = 1/4, linewidth = 3/4) +
  annotate(geom = "text", 
           x = omega, y = 0.4, label = "95% HDI", 
           color = "white") +
...
If you wanted a better sense of the phenomena, you could do a simulation. We'll make a custom simulation function to compute the modes from many random draws from our $\operatorname{Beta}(\theta \mid 15, 7)$ distribution, with varying $N$ values.
...
save(sim, file = "sims/sim07.01.rds")
```
...
sim |> 
  ...
  labs(x = "mode", 
       y = NULL,
       title = expression("Variability of the mode for simulations of "*beta(theta*'|'*15*', '*7)*", the true mode of which is .7"),
...
tibble(t     = 1:5e4,
  ...
  geom_path(color = "steelblue", linewidth = 1/4) +
  geom_point(alpha = 1/2, color = "steelblue", size = 1/2) +
  scale_x_continuous(expression(theta), breaks = 1:7) +
...
p <- list()
data <- tibble(position = 1:nslots, 
               prob     = position_vec)

# Loop through the requisite time indexes, and
...
>
> This technique is profoundly useful when the target distribution $P(\theta)$ is a posterior proportional to $p(D \mid \theta) p(\theta)$. Merely by evaluating $p(D \mid \theta) p(\theta)$, without normalizing it by $p(D)$, we can generate random representative values from the posterior distribution. This result is wonderful because the method obviates direct computation of the evidence $p(D)$, which, as you'll recall, is one of the most difficult aspects of Bayesian inference. By using MCMC techniques, we can do Bayesian inference in rich and complex models. It has only been with the development of MCMC algorithms and software that Bayesian inference is applicable to complex data analysis, and it has only been with the production of fast and cheap computer hardware that Bayesian inference is accessible to a wide audience. (p. 152, *emphasis* in the original)
...
### Metropolis algorithm applied to Bernoulli likelihood and beta prior {#sec-Metropolis-algorithm-applied-to-Bernoulli-likelihood-and-beta-prior}
...
That first part follows what Kruschke put in his script. I'm going to bundle the next large potion in a function, `my_metropolis()` which will make it easier to plug the code into the `purrr::map()` function.
...
### Prior, likelihood and posterior for two biases {#sec-Prior-likelihood-and-posterior-for-two-biases}
...
### The posterior via exact formal analysis {#sec-The-posterior-via-exact-formal-analysis}

The plots in the left column of Figure 7.5 are outside of my skill set. I believe they are referred to as wireframe plots and it's my understanding that **ggplot2** does not support wire-frame plots at this time. However, I can reproduce versions of the right hand column. For our initial attempt for the upper right corner, we'll simulate.
...
betas <- tibble(theta_1 = rbeta(1e5, shape1 = 2, shape2 = 2),
...
Look at that--no more unsightly white space! We'll need the `dbern()` function from back in Chapter 6 for the middle right of Figure 7.5.
...
With our trusty `dbern()` function in hand, we're almost ready to compute and plot the likelihood. We just need to define our data.
...
crossing(theta_1 = theta_sequence,
         theta_2 = theta_sequence) |>
  mutate(likelihood_1 = dbern(x = theta_1, z = z1, n = n1),
...
d_prior <- crossing(theta_1 = theta_sequence,
                    theta_2 = theta_sequence) |>
  mutate(prior_1 = dbeta(x = theta_1, shape1 = 2, shape2 = 2),
         prior_2 = dbeta(x = theta_2, shape1 = 2, shape2 = 2))

# This is a redo from one plot up, but saved as `d_likelihood`
d_likelihood <- crossing(theta_1 = theta_sequence,
                         theta_2 = theta_sequence) |>
  mutate(likelihood_1 = dbern(x = theta_1, z = z1, n = n1),
         likelihood_2 = dbern(x = theta_2, z = z2, n = n2))
...
### The posterior via the Metropolis algorithm {#sec-The-posterior-via-the-Metropolis-algorithm}
...
bivariate_dbern <- function(theta1, data1, theta2, data2) {
  z1 <- sum(data1)
  n1 <- length(data1)
  z2 <- sum(data2)
  n2 <- length(data2)
...
target_rel_prob <- function(theta1, data1, theta2, data2) {
  l <- bivariate_dbern(theta1, data1, theta2, data2)
...
```{r}
mh <- tibble(proposal_sd = c(0.02, 0.2)) |> 
...
Recall we might also compute the effective sample size estimates with functions from **posterior**, like at the end of @sec-Metropolis-algorithm-applied-to-Bernoulli-likelihood-and-beta-prior. Here are the results using `ess_mean()` and `ess_bulk()`.
...
Figure 7.7 is still out of my skill set. But let's fit the model with our primary package, **brms**. These, recall, are the data.
...
The other new thing is that instead of directly using the Bernoulli likelihood function, we'll instead be setting `family = binomial(link = "identity")`. Recall that the Bernoulli function is a special case of the binomial function for which $N = 1$, for which each data point is a discrete trial.^[Kruschke briefly addressed this in his footnote #2 in Section 6.2 (p. 126). We will connect the Bernoulli and binomial approaches more fully in @sec-Dichotomous-Predicted-Variable.] When each data point is an aggregate of multiple trials, we use the binomial likelihood, instead.
...
draws |> 
  ...
  stat_halfeye(point_interval = mode_hdi, .width = 0.95, fill = "steelblue") +
...
draws |> 
  ...
  stat_histinterval(point_interval = mode_hdi, .width = 0.95,
                    fill = "steelblue2", slab_color = "steelblue4", outline_bars = TRUE,
                    breaks = 40, normalize = "panels") +
...
```{r}
warmups <- data.frame(b_Intercept = c(
  fit7.2$fit@sim$samples[[1]]$b_Intercept[1:500],
  fit7.2$fit@sim$samples[[2]]$b_Intercept[1:500],
  fit7.2$fit@sim$samples[[3]]$b_Intercept[1:500])) |> 
  # wWe'll need to recapture the iteration and chain information
...
The [**bayesplot** package](https://github.com/stan-dev/bayesplot) [@R-bayesplot; @gabry2019visualization] makes it easier to reproduce some of the plots in Figure 7.10 with the `mcmc_trace()` function.
...
As an alternative, we can also extract the warmup draws from a `brm()` fit with the [**ggmcmc** package](https://cran.rstudio.com/package=ggmcmc) [@R-ggmcmc; @fernandezGGMCMCAnalysisofMCMC2016], which has a variety of convenience functions for working with MCMC chains. The `ggs()` function extracts the posterior draws, including `warmup`, and arranges them in a tidy tibble. With those in hand, we can now make a trace plot with warmup draws.
...
### MCMC accuracy {#sec-MCMC-accuracy}
...
lagged_draws <- draws |> 
  ...
  pivot_longer(cols = -.draw, names_to = "key") 
```
