# Metric Predicted Variable with Multiple Metric Predictors {#sec-Metric-Predicted-Variable-with-Multiple-Metric-Predictors}
...
set.seed(18)
d <- tibble(x_1 = runif(n = n, min = 0, max = 10),
...
For Figure 18.2, the $x$ variables look to be multivariate normal with a correlation of about -0.95. We can simulate such data with help from the [**MASS** package](https://CRAN.R-project.org/package=MASS) [@R-MASS; @MASS2002].
...
### The perils of correlated predictors {#sec-The-perils-of-correlated-predictors}
...
### The model and implementation {#sec-The-model-and-implementation}

Let's make our version of the model diagram in Figure 18.4 to get a sense of where we're going. If you look back to @sec-Robust-linear-regression, you'll see this is just a minor reworking of the code from Figure 17.2.
...
### The posterior distribution {#sec-The-posterior-distribution}
...
Like we did in @sec-Metric-Predicted-Variable-with-One-Metric-Predictor, let's wrap the consequences of Equation 18.1 into two functions.
...
stat_wilke <- function(height = 1.25, ...) {
  list(
    ...
    stat_halfeye(.width = 0, height = height, fill = "steelblue4", alpha = 0.6, ...),
    stat_pointinterval(.width = 0.95, color = "chocolate3", size = 2, point_size = 4),
    theme_minimal_hgrid()
  )
}
```
...
```{r}
nd <- tibble(prcnt_take_z = -1,
             spend_z      =  1)

fitted(fit18.1, newdata = nd)
```

#### $R^2$ is an effect size

Kruschke didn't use this language in the text, but the $R^2$ statistic is another kind of *effect size*. In the conventional OLS framework, $R^2$ is the proportion of variance in the criterion variable accounted for by the predictors in the model.^[It's also sometimes described as the squared multiple correlation coefficient.] It gives a sense of magnitude in that it ranges from 0 (no variance accounted for) to 1 (all the variance was accounted for). It's often used to compare models, though personally I'm less interested in the $R^2$ than I used to be.
...
draws |> 
  pivot_longer(cols = b_Intercept:b_prop_not_take_z) |> 
...
draws |> 
  set_names("Intercept~(beta[0])", "Spend~(beta[1])", "Percent~Take~(beta[2])", "Percent~Not~Take~(beta[3])", "sigma", "log[10](nu)") |> 
  ggpairs(upper = list(continuous = my_upper),
          diag = list(continuous = my_diag),
          lower = list(continuous = my_lower),
          labeller = label_parsed) +
...
```{r}
prior_draws <- prior_draws(fit18.2) |> 
  transmute(Intercept = make_beta_0(zeta_0 = b_Intercept,
                                    zeta_1 = b_spend_z,
                                    zeta_2 = b_prcnt_take_z,
                                    zeta_3 = b_prop_not_take_z,
...
We can't quite reproduce Figure 18.8 with our **ggplot2** repertoire. But we can capture some of Figure 18.8 with the `geom_raster()` approach we used in @sec-Overview-of-the-Generalized-Linear-Model.

```{r}
#| fig-width: 8
#| fig-height: 2.5

d <- crossing(x1 = seq(from = 0, to = 10, by = 0.5),
...
# Exponential density
p6 <- tibble(x = seq(from = 0, to = 1, by = 0.01)) |> 
  ggplot(aes(x = x, y = (dexp(x, 2) / max(dexp(x, 2))))) +
  geom_area(alpha = 0.6, color = "steelblue4", fill = "steelblue4") +
  annotate(geom = "text",
           x = 0.5, y = 0.2,
...
# Four annotated arrows
p9 <- tibble(x = c(0.43, 0.43, 1.5, 2.5),
             y = c(1, 0.55, 1, 1),
             xend = c(0.43, 1.225, 1.5, 1.75),
             yend = c(0.8, 0.15, 0.2, 0.2)) |>
  ggplot(aes(x = x, xend = xend,
...
draws |> 
  pivot_longer(cols = Spend:x_rand_12) |> 
...
```{r}
fit18.1 <- add_criterion(fit18.1, criterion = "loo")
...
```{r loo_fit18.9}
fit18.9  <- add_criterion(fit18.9,  criterion = "loo")
...
nd <- tibble(spend_z       = seq(from = -3, to = 3, length.out = n_points),
             prcnt_take_z  = 0, 
             stu_tea_rat_z = 0, 
             salary_z      = 0)
...
pp <- pp_average(
  fit18.1111, fit18.0111, fit18.1011, fit18.1101, fit18.1110, fit18.0011, fit18.0101, fit18.0110, 
...
# What vales of the predictors would we like to evaluate the weighted posterior over?
nd <- tibble(spend_z       = c(seq(from = -3, to = 3, length.out = n_points),
...
## Bonus: On transforming standardized parameters

We spent a lot of time and code in this chapter (and the last) transforming parameters fit on the standardized $\zeta$ scale to the un-standardized $\beta$ scale. Using Equation 18.1 from @sec-The-model-and-implementation, we made elaborate custom functions designed to help us with those transformations. These transformations served pedagogical purposes for the text, but they may not be necessary for your real-world use cases. Here are a few contexts where it may be fine to leave the parameters in their $\zeta$ scales:

* If your data are scaled with arbitrary metrics (such as many questionnaire items and sum scores).
* If the $\zeta$ scale is more meaningful to your target audience.
* If your focus is on variable selection, rather than on the parameters of the variables themselves.
* If you are primarily interested in prediction.

For the latter, we saw in @sec-Metric-Predicted-Variable-with-One-Metric-Predictor that one can make predictions on the non-standardized scale from a standardized model with two basic methods. The first method (*transform-then-predict*), and the one Kruschke focused on in the text, was to first transform the parameters themselves to the un-standardized metric, and then make predictions on the un-standardized scale. The second method (*predict-then-transform*) we briefly explored was to first make the predictions on the standardized scale, and then un-standardize the values of the predictor inputs and the predictions. In my experience, the *predict-then-transform* method is often faster, particularly for more complicated models.

Here's a quick example of the *predict-then-transform* method using `fit18.1`. We'll compute and plot the fitted values for `SATT` across the full observed range of `PrcntTake`, holding the other predictor `Spend` constant at its mean.
...
m_x <- mean(my_data$PrcntTake)
s_x <- sd(my_data$PrcntTake)
m_y <- mean(my_data$SATT)
s_y <- sd(my_data$SATT)

# Define the predictor grid with standardized inputs
nd <- tibble(
  prcnt_take_z = seq(from = min(my_data$prcnt_take_z), to = max(my_data$prcnt_take_z), length.out = 101),
  spend_z = 0)
